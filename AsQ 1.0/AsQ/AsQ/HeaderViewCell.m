...
- (id)initWithFrame:(CGRect)frame buttons:(PAPPhotoHeaderButtons)otherButtons {
    ...
    if (self) {
        ...
        // translucent portion
        self.containerView = [[UIView alloc] initWithFrame:CGRectMake( 20.0f, 5.0f, self.bounds.size.width - 20.0f * 2.0f, self.bounds.size.height-10)];
        [self addSubview:self.containerView];
        [self.containerView setBackgroundColor:[UIColor colorWithPatternImage:[UIImage imageNamed:@"BackgroundComments.png"]]];
        
        
        self.avatarImageView = [[ASQProfileImageView alloc] init];
        self.avatarImageView.frame = CGRectMake( 6.0f, 14.0f, 25.0f, 25.0f);
...
//            
//            // timestamp
//            self.timestampLabel = [[UILabel alloc] initWithFrame:CGRectMake(206.0f, 8.0f, containerView.bounds.size.width - 50.0f - 29.0f, 18.0f)];
...
- (void)setPhoto:(PFObject *)aPhoto {
    ...
    UILabel *authorNameLabel = [[UILabel alloc] initWithFrame:CGRectMake(8.0f, 5.0f, self.avatarImageView.bounds.size.width+120, 12.0f)];
    [authorNameLabel setTextAlignment:UITextAlignmentCenter];
    [authorNameLabel setBackgroundColor:[UIColor clearColor]];
    [authorNameLabel setTextColor:[UIColor blackColor]];
    [authorNameLabel setText:[user objectForKey:kPAPUserDisplayNameKey]];
    [authorNameLabel setFont:[UIFont boldSystemFontOfSize:12.0f]];
    [self.avatarImageView addSubview:authorNameLabel];
